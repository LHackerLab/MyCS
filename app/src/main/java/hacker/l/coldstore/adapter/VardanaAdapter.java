...
public class VardanaAdapter extends RecyclerView.Adapter<VardanaAdapter.MyViewHolder> {
    private Typeface materialdesignicons_font, ProximaNovaRegular;
    private Context mContext;
    private List<Result> userList, FilteruserList;
    VardanaFragment fragment;
    ProgressDialog pd;

    public VardanaAdapter(Context mContext, List<Result> userList, VardanaFragment fragment) {
    ...
    @Override
    public VardanaAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {

        View itemView = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.item_vardana, parent, false);

        return new VardanaAdapter.MyViewHolder(itemView);
    }


    @Override
    public void onBindViewHolder(VardanaAdapter.MyViewHolder holder, final int position) {
        if (position % 2 == 1) {
        ...
        holder.tv_edit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                fragment.updateVardanaData(true, FilteruserList.get(position).getName(), FilteruserList.get(position).getPhone(), FilteruserList.get(position).getQty(), FilteruserList.get(position).getType(), FilteruserList.get(position).getVardanaId());
...
//            if (result != null) {
            StringRequest stringRequest = new StringRequest(Request.Method.POST, Contants.SERVICE_BASE_URL + Contants.deleteVardana,
                    new Response.Listener<String>() {
                        @Override
                        public void onResponse(String response) {
                            pd.dismiss();
                            FilteruserList.remove(position);
                            fragment.setVardanaAdapter();
                ...
                protected Map<String, String> getParams() throws AuthFailureError {
                    ...
                    params.put("vardanaId", String.valueOf(FilteruserList.get(position).getVardanaId()));
                    return params;
                }
            };
...
//            }
    ...
    public class MyViewHolder extends RecyclerView.ViewHolder {
        TextView tv_name, tv_phone,tv_date, tv_qty, tv_type, tv_edit, tv_delete;
